...
namespace Embarkasi.Controllers
{
    public class AreaController : Controller
    {
        ...
        private string controller_name = "Area";
        private string title_name = "Area";
        ...
        public AreaController(AppDBContext context, ILogger<AreaController> logger)
        {
            _context = context;
            _logger = logger;
        }
        public IActionResult GetAll()
        {
            try
            {
                var data = _context.tbl_m_area.OrderBy(x => x.created_at).ToList();
        ...
        public IActionResult Get(int id)
        {
            try
            {
                var result = _context.tbl_m_area.Where(x => x.id == id).FirstOrDefault();
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Insert(tbl_m_area a)
        {
            try
            {

                _context.tbl_m_area.Add(a);
        ...
        [HttpPost]
        public ActionResult Update(tbl_m_area a)
        {
            try
            {
                var tbl_ = _context.tbl_m_area.FirstOrDefault(f => f.id == a.id);
                if (tbl_ != null)
                {
                    tbl_.id = a.id;
                    tbl_.area = a.area;
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                var tbl_ = _context.tbl_m_area.FirstOrDefault(f => f.id == id);
                if (tbl_ != null)
                {
                    _context.tbl_m_area.Remove(tbl_);
                    _context.SaveChanges();
