...
{
    public class RunningTextController : Controller
    {
        private readonly AppDBContext _context;
        private readonly ILogger<RunningTextController> _logger;
        private string controller_name = "RunningText";
        private string title_name = "RunningText";
        [Authorize]
        ...
        public RunningTextController(AppDBContext context, ILogger<RunningTextController> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public IActionResult GetAll()
        {
            try
            {
                var data = _context.tbl_m_running_text.OrderBy(x => x.id).ToList();
                return Json(new { success = true, data = data }); // Include success: true
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Get(int id)
        {
            try
            {
                var result = _context.tbl_m_running_text.Where(x => x.id == id).FirstOrDefault();
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Insert(tbl_m_running_text a)
        {
            try
            {

                _context.tbl_m_running_text.Add(a);
        ...
        [HttpPost]
        public ActionResult Update(tbl_m_running_text a)
        {
            try
            {
                var tbl_ = _context.tbl_m_running_text.FirstOrDefault(f => f.id == a.id);
                if (tbl_ != null)
                {
                    tbl_.id = a.id;
                    tbl_.text = a.text;
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                var tbl_ = _context.tbl_m_running_text.FirstOrDefault(f => f.id == id);
                if (tbl_ != null)
                {
                    _context.tbl_m_running_text.Remove(tbl_);
                    _context.SaveChanges();
